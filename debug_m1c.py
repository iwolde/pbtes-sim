...
for E in [1000, 900, 800, 700, 600, 500]:
    for T_bot in [450, 400]:
        ...
        ok, _, _ = solver.attempt_to_solve(sys, 'offdesign', 'base_design', '1', tries=3)
        status = 'OK' if ok and sys.network.converged else 'FAIL'
        t14 = sys.conn_14.T.val if hasattr(sys,'conn_14') and ok else '?'
        print(f'  E={E} T_bot={T_bot}: {status} T14={t14}')
