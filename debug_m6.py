"""Mode 6 audit: Full TES charge (Parallel, two independent cycles)."""
...
for E in [1000, 800, 600]:
    for T_bot in [450, 400]:
        ...
        ok, _, _ = solver.attempt_to_solve(sys, 'offdesign', 'base_design', '6', tries=3)
        ...
        Q_ptc = sys.ptc_field.Q.val/1e6 if ok and sys.ptc_field.Q.val else '?'
        print(f'  E={E} T_bot={T_bot}: {status} T02={t02} T14={t14} Q_PTC={Q_ptc}MW')
