...
class SolarThermalSystem:
    ...
    def create_network1(self):
        ...
        self.conn_9.set_attr(T0=500, h0=700)
        self.conn_10.set_attr(T0=500, h0=700)
    ...
    def create_network3(self):
        ...
        # 3) Create and add components
        self.comp2 = tpc.Compressor(label='Compressor_main')
        self.process_hx = tpc.SimpleHeatExchanger(label='Process_HX')
        self.preheater_hx = tpc.SimpleHeatExchanger(label='Preheater_HX')
        self.cycle_closer = tpc.CycleCloser(label='CycleCloser')
    ...
    def create_network6(self):
        ...
        # 3) Create and add components
        self.comp = tpc.Compressor(label='Compressor_main')
        self.comp2 = tpc.Compressor(label='Compressor_Dch_loop')
        self.process_hx = tpc.SimpleHeatExchanger(label='Process_HX')
        self.preheater_hx = tpc.SimpleHeatExchanger(label='Preheater_HX')
        self.cycle_closer = tpc.CycleCloser(label='CycleCloser')
    ...
    def set_operation_mode(self, TESmode='4', 
        ...
        if TESmode == '1':
            ...
            Q_ptc_min = 600*self.ptc_field.A.val*0.7
            Q_ptc_now = current_irr*self.ptc_field.A.val*0.7
            r_m_tes = (Q_ptc_now - Q_ptc_min)/Q_ptc_min
            ...
            self.conn_11.set_attr(T=TES_target, m=tpcn.Ref(self.conn_3, r_m_tes*1, 0))
            self.conn_16.set_attr(m=tpcn.Ref(self.conn_11, 3, 0))
...
class Solver:
    ...
    """
    def __init__(self, solar_thermal_system, system_mode='Full', file_path=None):
    ...
    def get_mode(self, irr, TES_profile, prev_TES_lay):
        ...
        if 420 > np.average(TES_profile):
            if irr > 0:
                return '6'
            else:
                return '4'
        if irr > 900:
            if self.solar_system.conexion_params['7_T'] > TES_top - 20:
                return '1'
            else: 
                return '2'
        elif irr > 300:
    ...
    def _iterate_tes_coupling(self, system,
        ...
        for iteration in range(max_iter):
            ...
            if status == 'converged':
                ...
                #print('T_tes_in ', T_tes_in)
                #print('T_tes_out ', T_tes_out)
                #print('m_tes_in ', m_tes_in)
                #print('TES new profile ', system.tes.profile)
                break
    ...
    # -------------------------------------------------------------------------
    def solve_network_steady(self, mode='design', TESmode='2', current_irr=0):
        """
        ...
        self.TES_lay = 'Charge'
        TESmode = self.get_mode(current_irr, self.solar_system.tes.profile, self.TES_lay)
    ...
    def run_quasi_steady_simulation(self, days_to_simulate = 10,
        ...
        for idx, row in tqdm(data_frame.iterrows(), total=len(data_frame), desc="Simulating"):
            ...
            if self.current_mode in ['1','2','5']:
                ...
                Q_ptc_kJ   = 0
                comp_power = self.solar_system.comp2.P.val 
            else:
                Q_ptc_kJ   = self.solar_system.ptc_field.Q.val * 3600.0
                comp_power = self.solar_system.comp.P.val + self.solar_system.comp2.P.val
            Q_ph_kJ = self.solar_system.preheater_hx.Q.val
            if Q_ph_kJ > 0:
    ...
    def compute_performance_metrics(self):
        ...
        total_ph_energy_kJ  = df['ph_energy_kJ'].loc[df['ph_energy_kJ'] > 0].sum()
        # Pump power (W) * 3600 s for each step (assuming dt=1h)
        total_comp_energy_kJ = (df['comp_power_W'] * 3600.0 * self.dt_hours).sum() / 1000
        ...
        spf = total_ptc_energy_kJ / (total_ptc_energy_kJ + total_ph_energy_kJ + total_comp_energy_kJ*1000) # fraction
