"""Test Mode 1 offdesign convergence with warm-start."""
...
print(f'Design done. kA={solver.charge_hx_kA:.0f}')

# Test offdesign at the most challenging E values
fails = 0
total = 0
for E in [1000, 800, 600, 500, 400]:
    for T_bot in [450, 400, 350]:
        for attempt in range(3):
            ...
            t0 = time.time()
            ok, n_att, _ = solver.attempt_to_solve(sys, 'offdesign', 'base_design', '1', tries=3)
            dt = time.time() - t0
            status = 'OK' if ok and sys.network.converged else 'FAIL'
            if not (ok and sys.network.converged):
                fails += 1
            iters = len(getattr(sys.network, 'checked_solves', [])) if hasattr(sys.network, 'checked_solves') else '?'
            print(f'  E={E:4d} T_bot={T_bot:3d} #{attempt}: {status:4s} ({dt:.1f}s, {n_att}att)')
