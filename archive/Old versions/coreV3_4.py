...
class SolarThermalSystem:
    ...
    def create_network1(self):
        ...
        #self.conn_7.set_attr(p0 = 50)
        #self.conn_11.set_attr(m0=10)
        #self.conn_2.set_attr(m0=10)
        #self.charge_tes_hx.set_attr(design=['ttd_u'],
                                    #offdesign=['kA'])
    ...
    def create_network5(self):
        ...
        self.conn_16.set_attr(p=self.conexion_params['16_p'], 
                              fluid=self.conexion_params['16_f'])
    def TES_ind(self, current_irr):
        Q_ptc_max = 600*self.ptc_field_A*0.7
        Q_ptc_now = current_irr*self.ptc_field_A*0.7
    ...
    def set_operation_mode(self, TESmode='4', current_irr=0):
        ...
        if TESmode == '1':
            ...
            r_m_tes = (Q_ptc_now - Q_ptc_max)/Q_ptc_max
            #print(r_m_tes)
            #self.conn_5.set_attr(m=tpcn.Ref(self.conn_11, r_m_tes*20, 0))
            self.conn_11.set_attr(T=self.conexion_params['7_T'], m=tpcn.Ref(self.conn_5, r_m_tes*0.25, 0))
            ...
            self.conn_5.set_attr(T=self.conexion_params['7_T']+10, m=None)
            self.discharge_tes_hx.set_attr(Q=None, pr1=0.99, pr2=0.99)
            self.conn_18.set_attr(m=tpcn.Ref(self.conn_5, 0.5, 0))
            #print(self.tes.state)
...
class Solver:
    ...
    def get_mode(self, irr, TES_profile, prev_TES_lay):
        ...
        if irr > 700:
            #print('s1')
            #print(TES_Tout)
            #print(self.solar_system.conexion_params['7_T'] )
            if self.solar_system.conexion_params['7_T'] > TES_top:
                #TES_ind = self.solar_system.TES_ind(irr)
                #print('s2')
                return '1'
                #if TES_ind:
                    #print('s3')
    ...
    def run_quasi_steady_simulation(self, days_to_simulate = 10,
        ...
        for idx, row in tqdm(data_frame.iterrows(), total=len(data_frame), desc="Simulating"):
            ...
            # Decide TES mode
            new_TESmode = self.get_mode(current_irr, self.solar_system.tes.profile, self.TES_lay)
            #print('Mode ', new_TESmode)
            #print('Irr ', current_irr)
            #print('Profile ', self.solar_system.tes.profile)
            ...
            #print('Profile again', self.solar_system.tes.profile)
