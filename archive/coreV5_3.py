...
class SolarThermalSystem:
    ...
    def solve_network(self, mode='design', design_path="base_design", TESmode='1'):
        """
        Resuelve la red en el modo indicado.
        - En 'design': resuelve y guarda `base_design_{TESmode}`.
        - En 'offdesign': carga `base_design_{TESmode}` como referencia.
        """
        name = f'base_design_{TESmode}'
        if mode == 'design':
            self.network.solve(mode='design', max_iter=100)
            self.network.save(name)
        else:
            self.network.solve(mode='offdesign', max_iter=100, design_path=name)
...
class Solver:
    ...
    def get_mode(self, irr, TES_profile, prev_TES_lay):
        # Mapear top/bottom coherente con el layout previo
        if prev_TES_lay == 'Charge':
        ...
        T_proc_out = self.solar_system.conexion_params['6_T']  # tu 6_T
    
        # 1) DNI alto: decidir primero entre 1 ó 2 (PRIORIDAD)
        if irr > 700:
            if TES_bot < T_proc_out + 40:
                if TES_bot > 450:
                    return '1'   # solar al proceso + carga TES
                else:
                    return '6'
            else:
                return '2'   # solo proceso (desenfoque para ajustar)
    
        # 2) DNI medio: por defecto ‘2’; permitir “quedarse” en 6 si venías en 6 y aún está frío el fondo
        if irr > 600:
            if self.prev_TESmode == '6' and TES_bot < 470:
                return '6'   # mantener ‘6’ mientras se templ(a) el fondo
            return '2'
    
        # 3) DNI bajo: decidir entre descargar TES, standby o auxiliar
        if TES_top > T_proc_out:
            return '3'       # descarga TES a proceso
    
        # Mantener ‘5’ si venías en 5 y sigue MUY frío (histeresis), o entrar a 5 si está bajo 360°C
        if self.prev_TESmode == '5' and TES_bot < 400:
            return '5'
        if TES_bot < 360:
            return '5'
    
        # Si el fondo está frío pero no hay sol suficiente ⇒ standby
        # (la condición TES_bot<470 ya no fuerza ‘6’ aquí porque irr ≤ 600)
        return '4'


    def get_system_mode(self, irr):
        if self.system_mode == 'Full':
            new_TESmode = self.get_mode(irr, self.solar_system.tes.profile, self.solar_system.tes.state.capitalize())
    ...
    def initialize_modes(self):
        ...
        # # Mode 5
        self.irr = 100
        self.tes_params['Initial temperature'] = 350
        self.solar_system = SolarThermalSystem(rows=1, 
                                    tes_params=self.tes_params,
                                    component_params = self.component_params,
                                    conexion_params = self.conexion_params,
        ...
        # Mode 6
        self.irr = 1000
        self.tes_params['Initial temperature'] = 380
        self.solar_system = SolarThermalSystem(rows=1, 
                                    tes_params=self.tes_params,
                                    component_params = self.component_params,
                                    conexion_params = self.conexion_params,
    ...
    def _iterate_tes_coupling(self, system,
                              ...
                              Tamb=25):
        """
        Iteración TES↔TESPy para un escenario 'steady' (o un paso de tiempo),
        **sin** alterar la especificación de la red (no agrega ni quita ecuaciones).
        Basada en tu versión operativa previa.
        Devuelve `iter_info` para el logging ya implementado aguas arriba.
        """
        ...
        conv_factors  = []
        self.TES_profiles = []
        mode_3_fail = False  # evita uso no inicializado; no afecta la red
        

        # --- logging compatible con CSV ---
        iter_info = {
            'initial_mode': TESmode,
            'final_mode': None,
            'status': None,              # 'converged' | 'diverged' | 'max_iter'
            'attempts': []               # lista de {mode, tespy_converged, iter_idx}
        }

        # 2) Inicializar el guess del lado TES-HX desde el perfil actual
        prof = system.tes.profile
        is_charge = (TESmode in ['1','2','5','6'])        # modos que cargan
        # Si el TES está en 'charge', el extremo caliente es prof[0]; si está en 'discharge', es prof[-1]
        if system.tes.state == 'charge':
            T_hot = prof[0]
        else:  # 'discharge'
            T_hot = prof[-1]
        
        if is_charge and TESmode != '2':
            # salida TES→HX en carga debe partir en el extremo caliente
            system.conn_13.set_attr(T=T_hot)
        elif TESmode in ['3']:
            # salida TES→HX en descarga también debe ser el extremo caliente (según estado)
            system.conn_15.set_attr(T=T_hot)
        
        old_profile = system.tes.profile
        #print('TESmode: ',TESmode)
        #print('old_profile: ',old_profile)
        # 3) Iteración
        for iteration in range(max_iter):
            #print('old_profile: ', iteration,' ', old_profile)
            # a) Resolver la red TESPy (sin tocar especificación)
            if mode == 'offdesign':
                system.network.set_attr(iterinfo=False)
            system.solve_network(mode=mode, design_path=design_path, TESmode=TESmode)

            # log del intento de solve
            ...
            # Revisar convergencia en HX de descarga (misma lógica previa)
            if TESmode in ['3']:
                ttd_min = system.discharge_tes_hx.ttd_min_func()
                if ttd_min <= 0:
                    mode_3_fail = True

            if not system.network.converged:
                if TESmode in ['1','6']:
                    print('System did not converge',TESmode, ' passing to 2')
                    TESmode = '2'
                ...
                elif TESmode in ['3','5'] or mode_3_fail:
                    print('System did not converge',TESmode, 'passing to 4')
                    TESmode = '4'
                    self.current_mode = TESmode
                    system.set_operation_mode(TESmode=TESmode, 
                                              current_irr=self.current_irr,
            ...
            # b) Leer nuevas condiciones del lado TES del HX
            if TESmode in ['1','5','6']:
            ...
            if m_tes_in < 0.01:
                self.mode_alert == True  # mantener tal cual estaba (no cambia ecuaciones)
                profile = system.tes.profile
                old_profile = system.tes.calc_heat_loss(profile, 3600, Tamb)
                system.tes.profile = old_profile
                self.TES_profiles.append(old_profile)
            ...
            system.tes.update_temperature_profile(T_in=T_tes_in,
                                                  mass_flow=m_tes_in,
                                                  initial_profile=old_profile)
            # usar el perfil actualizado como base para la siguiente iteración
            old_profile = system.tes.profile.copy()
            T_tes_out = system.tes.tout
            
            # d) Setear la nueva T de salida como borde para la próxima iteración
            if TESmode in ['1','5','6']:
            ...
            # e) Seguimiento de T_out para el criterio propio de acople
            ...
            # f) Evaluar criterio de convergencia del acople
            ...
            # Si no hubo 'break', se agotó max_iter
    ...
    def solve_network_steady(self, mode='design', TESmode='2'):
        ...
        iter_info = self._iterate_tes_coupling(mode=mode, system=self.solar_system,
    ...
    def _collect_step_signals(self, system, mode: str) -> dict:
        ...
        # ---------- estado del TES (para puertas lógicas) ----------
        tes_state = ''
        try:
        ...
        is_charge    = tes_state.startswith('charg')     # 'charge'
        is_discharge = tes_state.startswith('discharg')  # 'discharge'
        ...
        if is_charge:
            ...
            # standby / otros estados: si ambos muy pequeños, déjalos;
            # si uno domina 100×, anula el otro para limpiar ruido
            if q_chg_kw > 0 and q_dch_kw > 0:
                ratio = (max(q_chg_kw, q_dch_kw) / max(min(q_chg_kw, q_dch_kw), 1e-9))
                if ratio > 100:
                    if q_chg_kw > q_dch_kw:
                        q_dch_kw = 0.0
                    else:
                        q_chg_kw = 0.0
        ...
        try:
            ...
            if prof_raw is not None:
                prof = prof_raw[::-1] if is_charge else prof_raw
        ...
        if is_charge:
            ...
            # limpieza por umbral
            if mdot_tes_charge and mdot_tes_discharge:
                if abs(mdot_tes_charge) > 100*max(abs(mdot_tes_discharge), 1e-9):
                    mdot_tes_discharge = 0.0
                elif abs(mdot_tes_discharge) > 100*max(abs(mdot_tes_charge), 1e-9):
                    mdot_tes_charge = 0.0
    ...
    def run_quasi_steady_simulation(self, days_to_simulate = 10,
        ...
        for idx, row in tqdm(data_frame.iterrows(), total=len(data_frame), desc="Simulating"):
            ...
            self.results.append({
                ...
                'iter_status':         iter_info.get('status'),           # 'converged'|'diverged'|'max_iter'|'error'
                'attempt_count':       len(iter_info.get('attempts', [])),
                'attempted_modes':     [a.get('mode') for a in iter_info.get('attempts', [])],
                'attempts_json':       iter_info.get('attempts', []),     # lista de dicts (se serializa en CSV)
...
class Reporting:
    ...
    def plot_TES_profile_colormap(self, df_results):
        ...
        for i in range(N):
            ...
            if layout_i == 'discharge':
                temp_profile = np.array(list_element).ravel()
            elif layout_i == 'charge':
