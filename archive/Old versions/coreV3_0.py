...
class ThermalEnergyStorage:
    ...
    def __init__(self, tes_params, name, dt):
        ...
        self.name = name
        self.state = 'standby'  # default state
        self.inlet = 'top'
        self.outlet = 'bottom'
...
class SolarThermalSystem:
    ...
    def __init__(self, rows=1, modules=1, tes_params=None, T_set_tes = 470):
        """
    ...
    def create_network(self):
        ...
        self.network = tpn.Network(fluids=['Water'], T_unit='C', p_unit='bar', h_unit='kJ / kg')

        # 3) Create and add components
        self.comp = tpc.Compressor(label='Compressor_main')
        
        #self.comp_ch = tpc.Compressor(label='Compressor_charge')        
        #self.comp_dch = tpc.Compressor(label='Compressor_discharge')

        #self.gas_burner = tpc.CombustionChamber(label='GasBurner')
        ...
        #self.process_source = tpc.Source('cooling water source')
        #self.process_sink = tpc.Sink('cooling water sink')
        
        #self.amb_air_source = tpc.Source('Ambient air source')
        #self.fuel_source = tpc.Source('Fuel source')
        #self.gasses_sink = tpc.Sink('Exahust gasses sink')
        ...
        self.merge1 = tpc.nodes.merge.Merge(label='Merge1')
        self.splitter2 = tpc.nodes.splitter.Splitter(label='Splitter2')
        ...
        if self.tes_params is not None:
            # Create an instance of your ThermalEnergyStorage class
            self.tes = ThermalEnergyStorage(self.tes_params, name='MyTES', dt=self.TES_dt)
        else:
            self.tes = None
    ...
    def create_connections(self):
        """
        Creates the connections forming a loop:
          Compressor -> Parabolic Trough -> Process HX -> Compressor
        ...
        # # Process connections
        # self.conn_hx1 = tpcn.Connection(
        #     self.process_source, 'out1', 
        #     self.process_hx, 'in2', label='SOURCE_to_PR'
        # )
                
        # self.conn_hx2 = tpcn.Connection(
        #     self.process_hx, 'out2', 
        #     self.process_sink, 'in1', label='PR_to_SINK'
        # )

        # # Combustion connections                
        # self.conn_17 = tpcn.Connection(
        #     self.amb_air_source, 'out1', 
        #     self.gas_burner, 'in1',
        #     label = 'AMB_to_GB'
        # )

        # self.conn_18 = tpcn.Connection(
        #     self.fuel_source, 'out1', 
        #     self.gas_burner, 'in2',
        #     label = 'FUEL_to_GB'
        # )

        # # Preheater connections
        # self.conn_16 = tpcn.Connection(
        #     self.preheater_hx, 'out1', 
        #     self.gasses_sink, 'in1',
        #     label = 'GB_to_SINK'
        # )
    ...
    def set_operation_mode(self, TESmode='2'):
        """
        ...
        if TESmode == '1':
            ...
            self.ptc_field.set_attr(Tamb=20)
            self.conn_2.set_attr(T=None, m=None)
            self.conn_11.set_attr(T=430, m=None)
            self.conn_13.set_attr(T=None, m=0.0001)
            
            self.charge_tes_hx.set_attr(Q=None, pr1=None, pr2=0.99, eff_max=None)
            self.discharge_tes_hx.set_attr(Q=None, pr1=None, pr2=0.99, eff_max=0.5)
            
            self.conn_15.set_attr(m=5)
            self.conn_17.set_attr(m=0.0001)
            
        elif TESmode == '2':
            #self.ptc_field.set_attr(Tamb=400)
            # Divide flow between Process and TES
            self.conn_11.set_attr(T=None, m=0.0001)
            self.conn_13.set_attr(T=None, m=0.0001)
            
            self.charge_tes_hx.set_attr(Q=0, pr1=None, pr2=0.99, eff_max=None)
            self.discharge_tes_hx.set_attr(Q=0, pr1=None, pr2=0.99, eff_max=None)

            self.conn_15.set_attr(m=0.0001)
            self.conn_17.set_attr(m=0.0001)

        elif TESmode == '3':
            # All flow from TES, none in PTC and charge loop
            #self.conn_2.set_attr(T=420)
            self.ptc_field.set_attr(Tamb=300)
            self.conn_2.set_attr(T=350, m=0.0002)
            self.conn_11.set_attr(T=None, m=0.0001)
            self.conn_13.set_attr(T=self.TES_set_temp, m=None)
            
            self.charge_tes_hx.set_attr(Q=None, pr1=None, pr2=0.99, eff_max=None)
            self.discharge_tes_hx.set_attr(Q=None, pr1=None, pr2=0.99, eff_max=None)

            self.conn_15.set_attr(m=0.0001)
            self.conn_17.set_attr(m=8)
        elif TESmode == '4':
            self.ptc_field.set_attr(Tamb=400)
            self.conn_2.set_attr(T=None, m=0.0002)
            self.conn_11.set_attr(T=None, m=0.0001)
            self.conn_13.set_attr(T=None, m=None)
    ...
    def solve_network(self, mode='design', design_path="base_design"):
        """
        ...
        """
        #logging.info(f"Solving network in {mode} mode ...")

        if mode == 'design':
            self.network.solve(mode=mode)
            self.network.save("base_design")
        else:
            self.network.solve(mode=mode, design_path="base_design")
...
class Solver:
    ...
    """
    def __init__(self, solar_thermal_system):
    ...
    def gen_latex(self, path='report', filename='report.tex'):
        ...
        #ttdm.document_model(self.solar_system.network, path='report', filename='report.tex', fmt={})

        
    def get_mode(self, irr):
        return '1' if irr > 300 else 'discharge'
    ...
    # -------------------------------------------------------------------------
    def _iterate_tes_coupling(self, mode='design', TESmode='2', design_path='base_design'):
        """
        ...
        if not system.tes:
            # If no TES is defined, just solve once and return
            system.network.solve(mode='offdesign', design_path=design_path)
            if not system.network.converged:
                raise RuntimeError("TESPy solver did not converge (no TES).")
            return
        ...
        system.set_operation_mode(TESmode)
        ...
        if TESmode == '1':
            ...
            if system.TES_set_temp > T_in_top:
                print('Using mode 4')
                system.set_operation_mode('4')
        elif TESmode == '2':
            system.conn_15.set_attr(T=system.TES_set_temp)
            system.conn_17.set_attr(T=system.TES_set_temp)
        #self.gen_latex()
        old_profile = system.tes.profile
        # 3) Iteration loop
        for iteration in range(max_iter):
            # a) Solve the main TESPy network
            system.network.solve(mode=mode, design_path=design_path)
            if not system.network.converged:
                self.gen_latex()
            ...
            #print(T_tes_in, m_tes_in)
            # c) Update the 1D TES model
            ...
            #print('TES Tout ', T_tes_out)
            
            
            #print(system.tes.profile)
    ...
    # -------------------------------------------------------------------------
    def solve_network_steady(self, mode='design', TESmode='2', design_path="base_design"):
        """
        ...
        self._iterate_tes_coupling(mode=mode, TESmode=TESmode, design_path=design_path)

    def run_quasi_steady_simulation(self, data_frame, time_col='time', E_col='dni', Tamb_col='Tamb'):
        """
        ...
        for idx, row in tqdm(data_frame.iterrows(), total=len(data_frame), desc="Simulating"):
            ...
            # Decide charge vs discharge
            new_mode = self.get_mode(current_irr)
            if new_mode != self.current_mode:
                self.solar_system.set_operation_mode(TESmode=new_mode)
            ...
            if self.current_mode == 'charge':
                ...
                self.solar_system.ptc_field.set_attr(E=0, Tamb=current_Tamb)

            # For each time step, do the same iteration
            self._iterate_tes_coupling(TESmode=self.current_mode)

            # After final iteration for this time-step:
            Q_ptc_kJ   = self.solar_system.ptc_field.Q.val * 3600.0
            Q_gb_kJ    = self.solar_system.gas_burner.calc_ti() * 3600.0
            pump_power = self.solar_system.pump.P.val
            fuel_flow  = self.solar_system.conn_comb2.m.val_SI
    ...
    def compute_performance_metrics(self):
        ...
        total_gb_energy_kJ  = df['gb_energy_kJ'].sum()
        ...
        total_pump_energy_kJ = (df['pump_power_W'] * 3600.0 * self.dt_hours).sum()
        total_fuel_kg        = (df['fuel_flow_kg_s'] * 3600.0 * self.dt_hours).sum()

        # Convert kJ to MWh (1 MWh = 3.6e6 kJ)
        ptc_energy_MWh   = total_ptc_energy_kJ / 3.6e6
        gb_energy_MWh    = total_gb_energy_kJ  / 3.6e6
        pump_energy_MWh  = total_pump_energy_kJ / 3.6e6
        
        # Optional "solar plant factor" if user provided a process demand
        spf = total_ptc_energy_kJ / (total_ptc_energy_kJ + total_gb_energy_kJ) # fraction

        # Print performance summary line by line
        print("\n=== Performance Summary ===")
        print(f"Total PTC Energy:       {ptc_energy_MWh:6.2f} MWh")
        print(f"Total Gas Burner Energy:{gb_energy_MWh:6.2f} MWh")
        print(f"Total Pump Energy:      {pump_energy_MWh:6.2f} MWh")
        print(f"Total Fuel Consumption: {total_fuel_kg:6.2f} kg")
        if spf is not None:
...
class Reporting:
    ...
    def plot_results(self, df_results):
        ...
        plt.plot(df_results['time'], df_results['gb_energy_kJ'],  label='Gas Burner Energy [kJ]')
        plt.xlabel('Time')
        plt.ylabel('Energy [kJ]')
        plt.title('PTC and Gas Burner Energy vs. Time')
        plt.legend()
        plt.tight_layout()
        plt.show()
    ...
    def plot_TES_profile_colormap(self, df_results):
        ...
        M = len(profiles_list)  # number of time steps
        ...
        for i in range(M):
            ...
            temp_profile = np.array(profiles_list[i]).ravel()
    
            # Optional: sanity check that all profiles have same length
            if len(temp_profile) != N:
                raise ValueError(
        ...
        x = np.linspace(0, 1, N)  # normalized from 0..1
        ...
        if isinstance(times_raw[0], pd.Timestamp):
            ...
            y_vals = (df_results['time'] - t0).dt.total_seconds() / 3600.0
            y_label = 'Time [h]'
        else:
        ...
        # 9) Labels, title, etc.
        ax.set_xlabel('Normalized TES Height [-]')
